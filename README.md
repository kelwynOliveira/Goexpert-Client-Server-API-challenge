...
Since the project has two Go system two folders were created one for the "server" where the `server.go` file will be and other called "client" for the `client.go` file. Both will have the main package and they will start like this:
...
e.g.: `go mod init github.com/kelwynOliveira/Goexpert-Client-Server-API-challenge`
...
To develop this part of the project we must have in mind that:
